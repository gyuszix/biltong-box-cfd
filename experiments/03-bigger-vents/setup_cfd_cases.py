#!/usr/bin/env python3
"""
Generate the OpenFOAM case files (0/, constant/, system/) for experiment 3's
"side_bigvent" variant, and copy in the matching STL patches from
stl_out/side_bigvent/.
...
as experiment 1 (unchanged, so the comparison isolates vent size only). See
generate_geometry_v2.py for the box/fan geometry these numbers are derived
from.
"""
...
VARIANTS = {
    "side_bigvent": {
        "case_dir": os.path.join(ROOT, "case-side-bigvent"),
        "fan_dir": (0.0, -1.0, 0.0),  # fan on +y wall, blows in -y (same as exp1's side_mount)
    },
}
...
def main():
    ...
    print("\nDone. Next: ./run_cfd.sh side_bigvent")
