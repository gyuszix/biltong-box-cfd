#!/usr/bin/env python3
"""
Compare experiment 7's long_vents_lengthwise (this project's recommended
configuration: fan on -x short face, rods lengthwise, 12 vents split 6-and-6
across both long faces) against this experiment's fewer_vents_lengthwise
(identical fan/rods, but only 3 vents per long face instead of 6 - 6 total
instead of 12, same hole diameter). Renders airflow visualizations and
prints/saves quantitative metrics.

long_vents_lengthwise is NOT re-solved here - reused directly from
experiment 7's already-solved case. This isolates vent COUNT as the only
variable, holding fan position, rod orientation, and hole diameter fixed.

Run after fewer_vents_lengthwise has finished
(`./run_cfd.sh fewer_vents_lengthwise`):
...
VARIANTS = {
    "long_vents_lengthwise": {
        "label": "long_vents_lengthwise (exp 7 baseline: 6 vents per long face, 12 total)",
        "foam_file": os.path.join(EXP7_ROOT, "case-long-vents-lengthwise", "case-long-vents-lengthwise.foam"),
    },
    "fewer_vents_lengthwise": {
        "label": "fewer_vents_lengthwise (3 vents per long face, 6 total)",
        "foam_file": os.path.join(ROOT, "case-fewer-vents-lengthwise", "case-fewer-vents-lengthwise.foam"),
    },
}
...
def main():
    ...
    base = df[df.variant == "long_vents_lengthwise"].iloc[0]
    new = df[df.variant == "fewer_vents_lengthwise"].iloc[0]
    print("\n--- Verdict ---")
    winner = "fewer_vents_lengthwise" if new.rod_surface_speed_mean > base.rod_surface_speed_mean else "long_vents_lengthwise"
    print(
        f"Mean airflow speed at the rod/meat surface: long_vents_lengthwise="
        f"{base.rod_surface_speed_mean:.3f} m/s, fewer_vents_lengthwise="
        f"{new.rod_surface_speed_mean:.3f} m/s.\n"
        f"Uniformity (CV, lower=more even) at rod height: long_vents_lengthwise="
        f"{base.rod_height_slice_uniformity_cv:.2f}, fewer_vents_lengthwise="
        f"{new.rod_height_slice_uniformity_cv:.2f}.\n"
        f"Internal gauge pressure (mean): long_vents_lengthwise={base.internal_gauge_pressure_pa_mean:.1f} Pa, "
        f"fewer_vents_lengthwise={new.internal_gauge_pressure_pa_mean:.1f} Pa.\n"
