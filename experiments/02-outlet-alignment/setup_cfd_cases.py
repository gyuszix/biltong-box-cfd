#!/usr/bin/env python3
"""
Generate the OpenFOAM case files (0/, constant/, system/) for experiment 2's
"side_aligned" variant, and copy in the matching STL patches from
stl_out/side_aligned/.
...
as experiment 1 (unchanged, so the comparison isolates vent placement only).
See generate_geometry_v2.py for the box/fan geometry these numbers are
derived from.
"""
...
VARIANTS = {
    "side_aligned": {
        "case_dir": os.path.join(ROOT, "case-side-aligned"),
...
def main():
    ...
    print("\nDone. Next: ./run_cfd.sh side_aligned")
