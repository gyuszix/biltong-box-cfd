#!/usr/bin/env python3
"""
Generate the OpenFOAM case files (0/, constant/, system/) for both fan-mount
variants, and copy in the matching STL patches from stl_out/<variant>/.
...
so the assumptions are all in one place. See generate_geometry_v2.py for the
box/fan geometry these numbers are derived from.
"""
...
# variant name (matches stl_out/<name>/ and generate_geometry_v2.py) ->
# (case directory, fan-face outward normal direction, i.e. the direction the
#  fan blows INTO the box)
VARIANTS = {
    "side_mount": {
        "case_dir": os.path.join(ROOT, "case-side-mount"),
        "fan_dir": (0.0, -1.0, 0.0),  # fan on +y wall, blows in -y
    },
    "lid_mount": {
        "case_dir": os.path.join(ROOT, "case-top-mount"),
        "fan_dir": (0.0, 0.0, -1.0),  # fan on lid, blows down (-z)
    },
}
...
def main():
    ...
    print("\nDone. Next: ./run_cfd.sh side_mount && ./run_cfd.sh lid_mount")
