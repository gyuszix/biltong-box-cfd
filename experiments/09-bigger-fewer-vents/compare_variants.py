#!/usr/bin/env python3
"""
Three-way comparison: experiment 7's long_vents_lengthwise (this project's
recommended configuration: fan on -x short face, rods lengthwise, 12 vents
split 6-and-6 across both long faces, 15mm holes), experiment 8's
fewer_vents_lengthwise (identical fan/rods, only 6 vents total - 3 per long
face - same 15mm holes), and this experiment's big_vents_lengthwise
(identical fan/rods, same 6 total vents as exp8, but 21.21mm holes sized so
6 holes match exp7's total open vent area). Renders airflow visualizations
and prints/saves quantitative metrics.

long_vents_lengthwise and fewer_vents_lengthwise are NOT re-solved here -
reused directly from experiments 7 and 8's already-solved cases. This
isolates vent DIAMETER as the only variable relative to exp8, and vent
COUNT as the only variable relative to exp7.

Run after big_vents_lengthwise has finished
(`./run_cfd.sh big_vents_lengthwise`):
    .venv/bin/python compare_variants.py

Outputs go to results/:
    velocity_slice.png     - vertical mid-plane velocity magnitude, all 3 side by side
    streamlines.png        - fan-seeded streamlines through the rods, all 3 side by side
    rod_height_slice.png   - horizontal slice at rod height, all 3 side by side
...
EXP8_ROOT = os.path.join(ROOT, "..", "08-fewer-vents")
...
ROD_Y_FRACTIONS = [1 / 3, 2 / 3]  # unchanged in all three variants: rods lengthwise
MEAT_STANDOFF = 0.015

VARIANTS = {
    "long_vents_lengthwise": {
        "label": "long_vents_lengthwise (exp 7: 12 x 15mm holes)",
        "foam_file": os.path.join(EXP7_ROOT, "case-long-vents-lengthwise", "case-long-vents-lengthwise.foam"),
    },
    "fewer_vents_lengthwise": {
        "label": "fewer_vents_lengthwise (exp 8: 6 x 15mm holes)",
        "foam_file": os.path.join(EXP8_ROOT, "case-fewer-vents-lengthwise", "case-fewer-vents-lengthwise.foam"),
    },
    "big_vents_lengthwise": {
        "label": "big_vents_lengthwise (exp 9: 6 x 21.21mm holes, same area as exp 7)",
        "foam_file": os.path.join(ROOT, "case-big-vents-lengthwise", "case-big-vents-lengthwise.foam"),
    },
}
...
def main():
    ...
    exp7 = df[df.variant == "long_vents_lengthwise"].iloc[0]
    exp8 = df[df.variant == "fewer_vents_lengthwise"].iloc[0]
    exp9 = df[df.variant == "big_vents_lengthwise"].iloc[0]
    print("\n--- Verdict ---")
    print(
        f"Mean airflow speed at the rod/meat surface: exp7={exp7.rod_surface_speed_mean:.3f} m/s, "
        f"exp8={exp8.rod_surface_speed_mean:.3f} m/s, exp9={exp9.rod_surface_speed_mean:.3f} m/s.\n"
        f"Uniformity (CV, lower=more even) at rod height: exp7={exp7.rod_height_slice_uniformity_cv:.2f}, "
        f"exp8={exp8.rod_height_slice_uniformity_cv:.2f}, exp9={exp9.rod_height_slice_uniformity_cv:.2f}.\n"
        f"Internal gauge pressure (mean): exp7={exp7.internal_gauge_pressure_pa_mean:.1f} Pa, "
        f"exp8={exp8.internal_gauge_pressure_pa_mean:.1f} Pa, exp9={exp9.internal_gauge_pressure_pa_mean:.1f} Pa.\n"
        f"=> exp9's bigger holes should pull pressure back toward exp7 while keeping exp8's hole count, "
        f"if the orifice-flow theory behind this experiment holds."
    )
