#!/usr/bin/env python3
"""
Compare experiment 4's end_mount (rods crosswise - along y, perpendicular to
the fan's flow) against this experiment's rods_lengthwise (same fan-on-
short-face/vents-on-opposite-short-face layout, rods rotated 90 degrees to
run along x, parallel to the flow). Renders airflow visualizations and
prints/saves quantitative metrics.

end_mount is NOT re-solved here - reused directly from experiment 4's
already-solved case.

Run after rods_lengthwise has finished (`./run_cfd.sh rods_lengthwise`):
...
ROD_X_FRACTIONS = [1 / 3, 2 / 3]  # end_mount: rods crosswise, spaced across x, span y
ROD_Y_FRACTIONS = [1 / 3, 2 / 3]  # rods_lengthwise: rods spaced across y, span x
MEAT_STANDOFF = 0.015

VARIANTS = {
    "end_mount": {
        "label": "end_mount (exp 4 baseline: rods crosswise)",
        "foam_file": os.path.join(EXP4_ROOT, "case-end-mount", "case-end-mount.foam"),
        "rods_lengthwise": False,
    },
    "rods_lengthwise": {
        "label": "rods_lengthwise (rods parallel to the flow)",
        "foam_file": os.path.join(ROOT, "case-rods-lengthwise", "case-rods-lengthwise.foam"),
        "rods_lengthwise": True,
    },
}
...
def rod_positions(lengthwise):
    """
    Returns (fixed_coord, z, half_span) per rod. For crosswise rods (as in
    end_mount), fixed_coord is the x position and the rod spans y; for
    lengthwise rods, fixed_coord is the y position and the rod spans x.
    Mirrors generate_geometry_v2.py's rod builders in both experiments.
    """
...
def rod_ring_points(lengthwise, n_theta=24, n_along=14):
    r = ROD_DIAMETER / 2 + MEAT_STANDOFF
    thetas = np.linspace(0, 2 * np.pi, n_theta, endpoint=False)
    pts = []
    for coord, z, half_len in rod_positions(lengthwise):
        alongs = np.linspace(-half_len, half_len, n_along)
        for along in alongs:
            for th in thetas:
                if lengthwise:
                    # rod centered at y=coord, spans x, ring in the y-z plane
                    pts.append((along, coord + r * np.cos(th), z + r * np.sin(th)))
                else:
                    # rod centered at x=coord, spans y, ring in the x-z plane
                    pts.append((coord + r * np.cos(th), along, z + r * np.sin(th)))
    return np.array(pts)
...
def sample_rod_surface_speed(internal, lengthwise):
    pts = rod_ring_points(lengthwise)
...
def compute_metrics(name, cfg):
    ...
    rod_speed = sample_rod_surface_speed(internal, cfg["rods_lengthwise"])
...
def main():
    ...
    end = df[df.variant == "rods_lengthwise"].iloc[0]
    print("\n--- Verdict ---")
    winner = "rods_lengthwise" if end.rod_surface_speed_mean > base.rod_surface_speed_mean else "end_mount"
    print(
        f"Mean airflow speed at the rod/meat surface: end_mount="
        f"{base.rod_surface_speed_mean:.3f} m/s, rods_lengthwise="
        f"{end.rod_surface_speed_mean:.3f} m/s.\n"
        f"Uniformity (CV, lower=more even) at rod height: end_mount="
        f"{base.rod_height_slice_uniformity_cv:.2f}, rods_lengthwise="
        f"{end.rod_height_slice_uniformity_cv:.2f}.\n"
        f"Internal gauge pressure (mean): end_mount={base.internal_gauge_pressure_pa_mean:.1f} Pa, "
        f"rods_lengthwise={end.internal_gauge_pressure_pa_mean:.1f} Pa.\n"
