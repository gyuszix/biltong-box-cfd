#!/usr/bin/env bash
# Mesh + solve the fewer_vents_lengthwise variant with OpenFOAM, via Docker.
#
# Usage:
#   ./run_cfd.sh fewer_vents_lengthwise
#
# Requires setup_cfd_cases.py to have been run first (creates the case dir
...
VARIANT="${1:-}"
case "$VARIANT" in
    fewer_vents_lengthwise) CASE_DIR="case-fewer-vents-lengthwise" ;;
    *)
        echo "Usage: $0 <fewer_vents_lengthwise>" >&2
        exit 1
        ;;
esac
...
# empty marker file pyvista's/ParaView's OpenFOAMReader looks for
touch "$ROOT_DIR/$CASE_DIR/$CASE_DIR.foam"
