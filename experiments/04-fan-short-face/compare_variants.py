#!/usr/bin/env python3
"""
Compare experiment 1's side_mount (fan on long wall, vents split across
both short faces) against this experiment's end_mount (fan on one short
face, blowing down the long axis, all 12 vents consolidated onto the
opposite short face). Renders airflow visualizations and prints/saves
quantitative metrics.

side_mount is NOT re-solved here - reused directly from experiment 1's
already-solved case.

Run after end_mount has finished (`./run_cfd.sh end_mount`):
...
VARIANTS = {
    "side_mount": {
        "label": "side_mount (exp 1 baseline: fan on long wall)",
        "foam_file": os.path.join(EXP1_ROOT, "case-side-mount", "case-side-mount.foam"),
    },
    "end_mount": {
        "label": "end_mount (fan on short face, opposite vents)",
        "foam_file": os.path.join(ROOT, "case-end-mount", "case-end-mount.foam"),
    },
}
...
def main():
    ...
    end = df[df.variant == "end_mount"].iloc[0]
    print("\n--- Verdict ---")
    winner = "end_mount" if end.rod_surface_speed_mean > base.rod_surface_speed_mean else "side_mount"
    print(
        f"Mean airflow speed at the rod/meat surface: side_mount="
        f"{base.rod_surface_speed_mean:.3f} m/s, end_mount="
        f"{end.rod_surface_speed_mean:.3f} m/s.\n"
        f"Uniformity (CV, lower=more even) at rod height: side_mount="
        f"{base.rod_height_slice_uniformity_cv:.2f}, end_mount="
        ...
        f"end_mount={end.internal_gauge_pressure_pa_mean:.1f} Pa.\n"
