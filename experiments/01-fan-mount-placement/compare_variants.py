#!/usr/bin/env python3
"""
Compare the two solved CFD cases (side_mount vs lid_mount): render airflow
visualizations and print/save quantitative metrics to help pick a fan
placement.

Run after both variants have finished (`./run_cfd.sh side_mount` and
...
# ---------------------------------------------------------------------------
# Geometry constants (kept in sync with generate_geometry_v2.py / setup_cfd_cases.py)
# ---------------------------------------------------------------------------
CM = 0.01
...
MEAT_STANDOFF = 0.015  # assumed ~15mm out from rod centerline == hanging meat surface

VARIANTS = {
    "side_mount": {
        "label": "Side mount (fan on long wall)",
        "case_dir": os.path.join(ROOT, "case-side-mount"),
        "foam_file": os.path.join(ROOT, "case-side-mount", "case-side-mount.foam"),
    },
    "lid_mount": {
        "label": "Lid mount (fan on top, blows down)",
        "case_dir": os.path.join(ROOT, "case-top-mount"),
        "foam_file": os.path.join(ROOT, "case-top-mount", "case-top-mount.foam"),
    },
}
...
def rod_positions():
    """Returns [(x, z, half_length), ...] for the two rods, matching
    generate_geometry_v2.py's make_rods()."""
...
def rod_ring_points(n_theta=24, n_along=14):
    """Synthetic points on a cylindrical ring around each rod, at
    MEAT_STANDOFF beyond the rod surface - a stand-in for the surface of
    meat hanging on the rod, used to sample airflow speed there."""
...
def patch_flux(patch):
    """Net volumetric flow through a boundary patch (m^3/s), outward-positive."""
    ...
    if U is None:
        # fall back to point data averaged onto cells
...
RHO_AIR = 1.204  # kg/m3 at ~20C - only used to convert solver's kinematic p (m2/s2) to Pa
...
def compute_metrics(name, cfg):
    ...
    # internal gauge pressure (referenced to the outlet, which is fixedValue 0)
    # - tracked because a mildly positive box relative to ambient means any
    # leak at the lid seam pushes air out rather than drawing in dust/flies.
    # Opening up the vents (more area) will tend to pull this toward zero, so
    # it's worth watching alongside airflow speed, not just as an afterthought.
...
def add_streamlines(p, internal, boundary, cmap, clim):
    ...
    # nudge seeds slightly into the fluid domain along the fan's local normal
...
def main():
    ...
    # shared color scale across both variants for an apples-to-apples look
    clim = (0.0, 3.0)
    ...
    side = df[df.variant == "side_mount"].iloc[0]
    lid = df[df.variant == "lid_mount"].iloc[0]
    print("\n--- Verdict ---")
    winner = "side_mount" if side.rod_surface_speed_mean > lid.rod_surface_speed_mean else "lid_mount"
    print(
        f"Mean airflow speed at the rod/meat surface: side_mount="
        f"{side.rod_surface_speed_mean:.3f} m/s, lid_mount={lid.rod_surface_speed_mean:.3f} m/s.\n"
        f"Uniformity (coeff. of variation, lower=more even) at rod height: "
        f"side_mount={side.rod_height_slice_uniformity_cv:.2f}, "
        f"lid_mount={lid.rod_height_slice_uniformity_cv:.2f}.\n"
