#!/usr/bin/env python3
"""
Compare experiment 1's side_mount (15mm vent holes) against this
experiment's side_bigvent (20mm vent holes, same count/placement) - same
fan, same short-face placement (the placement experiment 2 showed wins),
only vent size changes. Renders airflow visualizations and prints/saves
quantitative metrics.

side_mount is NOT re-solved here - reused directly from experiment 1's
already-solved case.

Run after side_bigvent has finished (`./run_cfd.sh side_bigvent`):
...
VARIANTS = {
    "side_mount": {
        "label": "side_mount (exp 1 baseline: 15mm vents)",
        "foam_file": os.path.join(EXP1_ROOT, "case-side-mount", "case-side-mount.foam"),
    },
    "side_bigvent": {
        "label": "side_bigvent (20mm vents, same placement/count)",
        "foam_file": os.path.join(ROOT, "case-side-bigvent", "case-side-bigvent.foam"),
    },
}
...
def main():
    ...
    big = df[df.variant == "side_bigvent"].iloc[0]
    print("\n--- Verdict ---")
    winner = "side_bigvent" if big.rod_surface_speed_mean > base.rod_surface_speed_mean else "side_mount"
    print(
        f"Mean airflow speed at the rod/meat surface: side_mount(15mm)="
        f"{base.rod_surface_speed_mean:.3f} m/s, side_bigvent(20mm)="
        f"{big.rod_surface_speed_mean:.3f} m/s.\n"
        f"Uniformity (CV, lower=more even) at rod height: side_mount="
        f"{base.rod_height_slice_uniformity_cv:.2f}, side_bigvent="
        f"{big.rod_height_slice_uniformity_cv:.2f}.\n"
        f"Internal gauge pressure (mean): side_mount={base.internal_gauge_pressure_pa_mean:.1f} Pa, "
        f"side_bigvent={big.internal_gauge_pressure_pa_mean:.1f} Pa.\n"
