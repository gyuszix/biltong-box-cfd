#!/usr/bin/env python3
"""
Generate the OpenFOAM case files (0/, constant/, system/) for experiment 5's
"rods_lengthwise" variant, and copy in the matching STL patches from
stl_out/rods_lengthwise/.
...
and fan/vent placement as experiment 4's end_mount (unchanged, so the
comparison isolates rod orientation only). See generate_geometry_v2.py for
the box/fan geometry these numbers are derived from.
"""
...
VARIANTS = {
    "rods_lengthwise": {
        "case_dir": os.path.join(ROOT, "case-rods-lengthwise"),
...
def main():
    ...
    print("\nDone. Next: ./run_cfd.sh rods_lengthwise")
