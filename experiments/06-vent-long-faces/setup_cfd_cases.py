#!/usr/bin/env python3
"""
Generate the OpenFOAM case files (0/, constant/, system/) for experiment 6's
"long_face_vents" variant, and copy in the matching STL patches from
stl_out/long_face_vents/.
...
and fan position as experiment 4's end_mount (unchanged, so the comparison
isolates vent placement only). See generate_geometry_v2.py for the box/fan/
vent geometry these numbers are derived from.
"""
...
VARIANTS = {
    "long_face_vents": {
        "case_dir": os.path.join(ROOT, "case-long-face-vents"),
...
def main():
    ...
    print("\nDone. Next: ./run_cfd.sh long_face_vents")
