#!/usr/bin/env python3
"""
Compare experiment 1's side_mount (perpendicular vents, on both short end
faces) against this experiment's side_aligned (vents relocated to the wall
directly opposite the fan) - same fan, same total vent area, only placement
changes. Renders airflow visualizations and prints/saves quantitative
metrics to help decide which vent placement to carry into experiment 3.

side_mount is NOT re-solved here - reused directly from experiment 1's
already-solved case, so this only computes the one new run's cost.

Run after side_aligned has finished (`./run_cfd.sh side_aligned`):
...
EXP1_ROOT = os.path.join(ROOT, "..", "01-fan-mount-placement")
...
VARIANTS = {
    "side_mount": {
        "label": "side_mount (exp 1 baseline: vents on both short faces)",
        "foam_file": os.path.join(EXP1_ROOT, "case-side-mount", "case-side-mount.foam"),
    },
    "side_aligned": {
        "label": "side_aligned (vents on wall opposite the fan)",
        "foam_file": os.path.join(ROOT, "case-side-aligned", "case-side-aligned.foam"),
    },
}
...
def rod_positions():
    ...
    generate_geometry_v2.py's make_rods() (unchanged from experiment 1)."""
...
# ---------------------------------------------------------------------------
# Visualizations (identical approach to experiment 1)
# ---------------------------------------------------------------------------
...
def main():
    ...
    base = df[df.variant == "side_mount"].iloc[0]
    aligned = df[df.variant == "side_aligned"].iloc[0]
    print("\n--- Verdict ---")
    winner = "side_aligned" if aligned.rod_surface_speed_mean > base.rod_surface_speed_mean else "side_mount"
    print(
        f"Mean airflow speed at the rod/meat surface: side_mount(perpendicular)="
        f"{base.rod_surface_speed_mean:.3f} m/s, side_aligned(opposite wall)="
        f"{aligned.rod_surface_speed_mean:.3f} m/s.\n"
        f"Uniformity (CV, lower=more even) at rod height: side_mount="
        f"{base.rod_height_slice_uniformity_cv:.2f}, side_aligned="
        f"{aligned.rod_height_slice_uniformity_cv:.2f}.\n"
        f"Internal gauge pressure (mean): side_mount={base.internal_gauge_pressure_pa_mean:.1f} Pa, "
        f"side_aligned={aligned.internal_gauge_pressure_pa_mean:.1f} Pa.\n"
